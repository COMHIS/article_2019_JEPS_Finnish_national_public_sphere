...
npplaces <- read.csv("~/tyo/data-public/finnish-newspapers/unified/publication_locations-utf8.csv")
...
ggsave("~/Downloads/number-of-newspapers-by-town.png", plot, width = 7, height = 4, dpi=300)
library(cowplot)
...
data <- fnpplaces %>% filter(byear>=1850,byear<=1920) %>% mutate(location=if_else(KAUPUNKI_NORM %in% c("Viipuri","Turku","Oulu","Helsinki","Kuopio","Kotka","Pori","Porvoo","Tampere","Vaasa"),as.character(KAUPUNKI_NORM),"others"))
gplot(data, aes(group=location,x=location,y=lifetime)) + geom_boxplot() + geom_jitter(data=data %>% filter(lifetime==Inf))